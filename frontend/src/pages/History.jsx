...
const History = () => {
  ...
  useEffect(() => {
    // Хэрэглэгч нэвтэрсэн эсэхийг баталгаажуулж байж датагаа татна
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '50px' }}>Ачаалж байна...</div>;

  return (
    <div style={{ padding: '30px', maxWidth: '900px', margin: '0 auto', fontFamily: "'Montserrat', sans-serif" }}>
      <h2 style={{ color: '#321D73', marginBottom: '30px' }}>📝 Миний TOPIK 54 эссэний түүх</h2>
      ...
      {Array.isArray(history) && history.length > 0 ? (
        history.map((item) => (
          <div key={item._id} style={historyCardStyle}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <span style={dateStyle}>📅 {new Date(item.createdAt).toLocaleString('mn-MN')}</span>
                <h3 style={{ margin: '5px 0 0 0', color: '#1E293B', fontSize: '18px' }}>
                  {item.title || "Гарчиггүй эссэ"}
                </h3>
              </div>
            ...
            <div style={essayPreviewStyle}>
              <strong>Миний бичсэн:</strong><br/>
              {item.essayText?.substring(0, 150)}...
            </div>

            {/* Дэлгэрэнгүй засалт харах хэсэг */}
            <details style={{ marginTop: '15px' }}>
              <summary style={viewMoreButtonStyle}>
                👨‍🏫 Багшийн оноо болон дэлгэрэнгүй засалтыг харах
              </summary>
        ...
        <p style={{ textAlign: 'center', color: '#64748B', marginTop: '50px' }}>Түүх одоогоор алга байна.</p>
      )}
    </div>
  );
};
